...
export const getPackageAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const studentsCompleted = [...new Set(completedAttempts.map(a => (a.student as any)._id.toString()))].length;
    ...
    const averageTimeSpent = attemptsWithScores.length > 0
    ...
    const passRate = attemptsWithScores.length > 0
  ...
  try {
    ...
    const exportData = attempts.map(attempt => {
      ...
      return {
        ...
        successStatus: cmi.success_status || (cmi.lesson_status === 'passed' ? 'passed' : 'unknown'),
      };
    });
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="scorm-export-${Date.now()}.json"`);
...
export const getCompletionRates = async (req: Request, res: Response) => {
  try {
    ...
    const completedStudents = [...new Set(
      attempts.filter(isAttemptCompleted).map(a => a.student.toString())
    )];
...
export const getTimeAnalytics = async (req: Request, res: Response) => {
  try {
    ...
    const timeByStatus = {
      completed: {
        average: completedAttempts.length > 0 ? calculateTimeStatistics(completedAttempts).average : 0,
        total: calculateTotalTimeSpent(completedAttempts),
      },
      incomplete: {
        average: incompleteAttempts.length > 0 ? calculateTimeStatistics(incompleteAttempts).average : 0,
...
export const getInteractionData = async (req: Request, res: Response) => {
  try {
    ...
    const correctInteractions = interactions.filter((i: any) => 
      i.result === 'correct' || i.result === 'true'
    ).length;
    const incorrectInteractions = interactions.filter((i: any) => 
      i.result === 'incorrect' || i.result === 'false'
    ).length;
