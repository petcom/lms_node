# Instructor Publish/Unpublish Endpoints

## Summary
Add explicit publish/unpublish endpoints for SCORM packages so instructors/admins can control visibility without generic updates. Endpoints should be authenticated, idempotent, and enforce ownership/visibility, with clear errors and optional async side-effects.

## Requirements
- **Auth**: Require Bearer token with role `instructor` or `admin`; return 401 if missing/invalid token, 403 if role not allowed.
- **Existence/Ownership**: 404 if package does not exist or is not visible to the requesting instructor (e.g., instructor not owner/uploader and lacks admin role).
...
- **POST /api/v1/scorm/packages/:id/publish**
  ...
  - Behavior: set `isPublished = true`, `status = 'published'`, set `publishedBy` (instructor/admin id + role) and `publishedAt = now`.
...
- Add audit fields to `ScormPackage`:
  - `publishedBy: ObjectId refPath ['Admin','Instructor']`
...
## Controller Logic
1. **Auth/Role**: Use `isAuthenticated()` + `isInstructorOrAdmin` middleware.
2. **Lookup**: Find package by id; if not found return 404. If user is instructor and not uploader/owner (or otherwise not allowed), also return 404 to avoid information leakage.
...
- Add routes in `routes/scorm/scormPackageRoutes.ts`:
  - `POST /:id/publish`
  - `POST /:id/unpublish`
  - Both behind `isAuthenticated()` and `isInstructorOrAdmin`.
...
- Integration tests (phase2 scope):
  ...
  - 404 when package does not exist or instructor lacks visibility.
  - 401/403 when missing/invalid role.
  - Optional 409 when manifest missing/constraint fails.

## Future Considerations
- Hook into UI to toggle publish state from instructor/admin dashboard.
