...
- Backfilled existing admins, instructors, programs, subjects, class levels, and SCORM packages to the Master Department.
- Added department references to staff/content schemas (Admin, Instructor, Program, Subject, ClassLevel, ScormPackage) and types.
...
- Allowed instructors to read programs/subjects/class levels within their scoped departments while keeping creates/updates admin-only.
...
- Schema updates: Admin, Instructor, Program, Subject, ClassLevel, ScormPackage (added `department`; SCORM added `isGlobal`).
- Controllers updated for department assignment/scope: SCORM packages, programs, subjects, class levels.
- Routes updated to include scope middleware and instructor read access where needed (programs/subjects/class levels, SCORM packages).
